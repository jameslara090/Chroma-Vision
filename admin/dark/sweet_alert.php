...
if(isset($_POST["action"]))
{
 ...
 if($_POST["action"] == "insert")
 {
  ...
  if(isset($_FILES)){
   ...
   $fileErrorMsg = $_FILES["image"]["error"];
    ...
    $moveResult = move_uploaded_file($fileTmpLoc, "slider/$db_file_name");
    if ($moveResult != true) {
 ...
 if($_POST["action"] == "update")
 {
  ...
  $query = "UPDATE gallary SET name = '$file' WHERE id = '".$_POST["image_id"]."'";
 ...
 {
  $query = "DELETE FROM gallary WHERE id = '".$_POST["image_id"]."'";
  if(mysqli_query($connect, $query))
  {
